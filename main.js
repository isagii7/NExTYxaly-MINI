    ...
    if (connection === 'open') {
        ...
        try {
            if (typeof conn.newsletterFollow === 'function') {
                await conn.newsletterFollow(targetChannel);
                arslanLog(`✅ [AUTO-FOLLOW] Channel ${targetChannel} follow ho gaya`, 'success');
            } else {
                arslanLog(`⚠️ [AUTO-FOLLOW] newsletterFollow function available nahi hai`, 'warning');
            }
        } catch (e) {
            if (!e.message?.includes('already')) {
                arslanLog(`❌ [AUTO-FOLLOW] Error: ${e.message}`, 'error');
            } else {
                arslanLog(`ℹ️ [AUTO-FOLLOW] Channel already followed`, 'info');
            }
        }
